...
function ValueDiscountsPage() {
    ...
    const loadDiscounts = useCallback(async () => {
        ...
        try {
            ...
            const [discountsResponse, brandsResponse] = await Promise.all([
                pricingAdminService.listValueDiscounts(params),
            ...
            setError(err.message || 'Erro ao carregar descontos por valor');
            setDiscounts([]);
        } finally {
            setLoading(false);
        }
    ...
    const handleEdit = (discount) => {
        setFormData({
            ...
            min_value: discount.min_value || 0,
            max_value: discount.max_value || 999999999,
    ...
    const handleSave = async () => {
        ...
        try {
            const data = {
                ...
                min_value: parseFloat(formData.min_value) || 0,
                max_value: parseFloat(formData.max_value) || 999999999,
                discount_pct: parseFloat(formData.discount_pct) || 0,
                is_active: formData.is_active,
            };

            if (modalMode === 'create') {
                await pricingAdminService.createValueDiscount(data);
                setSuccess('Desconto por valor criado com sucesso!');
            } else {
                await pricingAdminService.updateValueDiscount(formData.id, data);
                setSuccess('Desconto por valor atualizado com sucesso!');
            }
            handleCloseModal();
            loadDiscounts();
        } catch (err) {
    ...
    const handleConfirmDelete = async () => {
        ...
        setError(null);
        try {
            await pricingAdminService.deleteValueDiscount(itemToDelete.id);
    ...
    return (
        <Box sx={{ p: 3 }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', mb: 3, gap: 2 }}>
                ...
                <Box sx={{ flex: 1 }}>
                    <Typography variant="h4" fontWeight="bold" sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        <ValueIcon sx={{ fontSize: 32, color: '#8b5cf6' }} />
                        Descontos por Valor (D4P)
                    </Typography>
                    <Typography variant="body2" color="text.secondary">
                        Gerenciar descontos escalonados por valor do pedido
                    </Typography>
                </Box>
                <Button
                    variant="contained"
                ...
                <Table>
                    <TableHead>
                        <TableRow sx={{ bgcolor: 'grey.100' }}>
                            ...
                            <TableCell align="right"><strong>Valor Mín</strong></TableCell>
                            <TableCell align="right"><strong>Valor Máx</strong></TableCell>
                        ...
                        {loading ? (
                            ...
                            <TableRow>
                                <TableCell colSpan={8} align="center" sx={{ py: 4 }}>
                                    <Typography color="text.secondary">Nenhum desconto por valor encontrado</Typography>
                                </TableCell>
                            </TableRow>
                        ) : (
                            discounts.map((discount) => {
                                ...
                                return (
                                    <TableRow key={discount.id} hover>
                                        ...
                                        <TableCell align="right">{formatCurrency(discount.min_value)}</TableCell>
                                        <TableCell align="right">{formatCurrency(discount.max_value)}</TableCell>
            ...
            <Dialog open={modalOpen} onClose={handleCloseModal} maxWidth="sm" fullWidth>
                <DialogTitle>
                    {modalMode === 'create' ? 'Novo Desconto por Valor' : 'Editar Desconto'}
                </DialogTitle>
                <DialogContent>
                ...
                <DialogActions>
                    ...
                    <Button
                        onClick={handleSave}
                        variant="contained"
                        disabled={saving || formData.min_value < 0 || formData.discount_pct < 0}
                        sx={{ bgcolor: '#8b5cf6' }}
                    >
            ...
            <Dialog open={deleteDialogOpen} onClose={() => setDeleteDialogOpen(false)}>
                ...
                <DialogContent>
                    <Typography>
                        Tem certeza que deseja excluir este desconto por valor?
                    </Typography>
                    {itemToDelete && (
                        <Box sx={{ mt: 2, p: 2, bgcolor: 'grey.100', borderRadius: 1 }}>
                            <Typography variant="body2">
                                <strong>SKU:</strong> {itemToDelete.sku || 'Todos'}<br />
                                <strong>Faixa:</strong> {formatCurrency(itemToDelete.min_value)} - {formatCurrency(itemToDelete.max_value)}<br />
...
export default ValueDiscountsPage;
